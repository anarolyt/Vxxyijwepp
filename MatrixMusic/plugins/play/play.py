    ...
    [
        [
            InlineKeyboardButton(   
              text=f"يوميات منتحر 🖤", url=f"https://t.me/CGC_l",)                        
        ],        
    ]
)
async def check_is_joined(message):    
    try:
        ...
        status = await app.get_chat_member("CGC_l", userid)
        return True
    except Exception:
        await message.reply_text(f'┇عزيزي: {message.from_user.mention}\n┇أشتࢪك في قناة البوت أولاً.\n┇قناة البوت: @CGC_l 🍓. ',reply_markup=force_btn,disable_web_page_preview=False)
        return False
